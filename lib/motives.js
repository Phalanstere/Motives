...
this.export_json = function() {
	...
	if (!doc) {
		alert("sollte das Element erzeugen");
	}
...
this.load_files = function() {
	var html = fs.readFileSync('files/test.txt', 'utf8');
	console.log(html);
...
this.show_files = function(files) {
 ...
 this.copy_to_clipboard = function() {
	var node = document.getElementById("EventRecorderData" ), range;
