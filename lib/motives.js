...
function Association(no, obj, parent){
	...
	this.edit = function() {
	    ...
	    s += '<label for="AssocLink">link<input id="AssocLink" value = "' + self.link + '"/><span id ="LinkTrigger">&#10026</span></label><br>';
      ...
      $("#AssociationInput #LinkTrigger").click(function(){
         if (self.texter) {
             self.texter.open_file(self.link);
         } else {               
            Texter.open_file(self.link);
         }
...
this.change_title = function() {
    ...
    $(".motives_prompt .submit").click(function(){
        ...
        self.db.update({ _id: self.id }, { $set: { title: self.title } }, { multi: false }, function (err, numReplaced) {
            ...
            $(".motives_prompt").fadeOut();      
            $("#motives_page #info").html(self.title);
            
            no = self.get_project(self.id);
            self.load_ndb(self.id);
            
            /*
            var dom = document.getElementById("select_project");
            dom.options[no].value = self.title;
...
this.update_projects = function() {
	...
	$("#select_project").unbind('change');
	$("#select_project").change(function(){
		var x = parseInt( $(this).val() , 10);
		console.log("Projektwechsel " + self.projects.length);
...
this.new_association = function() {
  ...
  $("#AssociationInput #AssociationSubmit").click(function(){
	obj = {};
	obj.div			= self.div_id;
	obj.title		= $("#AssociationInput #AssocTitle").val();
	obj.abstract	= $("#AssociationInput #AssocAbstract").val();	
	obj.fulltext	= $("#AssociationInput #AssocFulltext").val();	
	obj.link		= $("#AssociationInput #AssocLink").val();	
	obj.color		= $("#AssociationInput #AssociationInput #Color").val();
...
this.importer = function() {
  var   url = './files/test.json',
        i, 
        doc;
  
  
  $.getJSON(url, function(data) {
            alert("JSON eingeladen " + data.length);
        ...
        console.log("SPEICHERN DER PROJEKTE");   
...
this.import_json = function() {
  ...
  var s = '<div id = "IMPORTER" class = "motives_prompt">';
    s += '<input id = "ImportFile" type="file" name="file" id="file" />';
  s += '</div>';
  ...
  $("#ImportFile").change(function(){
        ...
        $.getJSON(url, function(data) {
            ...
            //myItems = data.items;
        // alert("Anzahl Projekte " + self.projects.length);
        console.log("SPEICHERN DER PROJEKTE");   
...
this.menu = function(){
    ...
    // s += '<div id = "import_json" title = "imports projects as json file" class = "motives_block">json_import</div>';
    ...
    // exports json;    
    $("#" + self.menu_id + " #import_json").click(function(){
        self.import_json();
    });
...
this.select_project_by_id = function(id) {
...
this.select_project = function (no) {
    ...
    self.db.find({_id: temp._id}, function (err, docs) {
        ...
        console.log("Anzahl Assoziationen " + loaded.associations.length);
